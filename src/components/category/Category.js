...
const Category = () => {
  ...
  const deleteCategory = (id) => {
    ...
    CategoryService.remove(itemDelete)
      .then((res) => {
        setCategories(
          categories.filter((category) => category._id !== itemDelete)
        );
        // setCount(count - 1);
        setIsLoading(false);
      ...
      <div className="grid grid-cols-12 gap-6 mt-5">
        <div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
          ...
          <div className="hidden md:block mx-auto text-slate-500">
            {/* Hiển thị {categories.length} / {count} danh mục cha */}
            Hiển thị {PAGE_SIZE} / {categories.length}{" "}
