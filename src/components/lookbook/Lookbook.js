...
import LookbookTable from "./LookbookTable";
import LookbookModal from "./LookbookModal";
import LookbookImageModal from "./LookbookImageModal";
import LookbookService from "../../services/lookbook.service";
import LookbookImageService from "../../services/lookbookImage.service";
import ConfirmDialog from "../shared/ConfirmDialog";

const TITLE = "Quản Lý Lookbook";
const TITLE_NAME = "Lookbook";
...
const Lookbook = () => {
  ...
  const handleShow = async (action, show, id, type = [0]) => {
    const data = id ? await getLookbook(id) : {};
    setLookbook(data);
    setAction(action);
    setShow(show);
    setShowType(type);
  ...
  const getLookbook = async (id) => {
    const data = await LookbookService.getById(id);
    return data.data;
  };

  const createLookbook = (lookbook) => {
    LookbookService.create(lookbook)
      .then((res) => {
        setLookbooks([res.data, ...lookbooks]);
        // setCount(count + 1);
        setIsLoading(false);
  ...
  const updateLookbook = (id, lookbook) => {
    LookbookService.update(id, lookbook)
      .then((res) => {
        setLookbooks(
          lookbooks.map((item) => (item._id == res.data._id ? res.data : item))
        );
        setIsLoading(false);
        setShow(false);
  ...
  const deleteLookbook = (id) => {
    ...
    setIsLoading(true);
    LookbookService.remove(itemDelete)
      .then((res) => {
        setLookbooks(
          lookbooks.filter((lookbook) => lookbook._id !== itemDelete)
        );
        // setCount(count - 1);
        setIsLoading(false);
  ...
  const createLookbookImage = (lookbookImage) => {
    LookbookImageService.create(lookbookImage)
      .then((res) => {
        lookbook.lookbookImages = [res.data, ...lookbook.lookbookImages];
  ...
  const updateLookbookImage = (id, lookbookImage) => {
    LookbookImageService.update(id, lookbookImage)
      .then((res) => {
        lookbook.lookbookImages = lookbook.lookbookImages.map((item) =>
  ...
  const deleteLookbookImage = (id) => {
    ...
    setIsLoading(true);
    // setIsDeleted(true);
    LookbookImageService.remove(itemDelete)
      .then((res) => {
        lookbook.lookbookImages = lookbook.lookbookImages.filter(
  ...
  return (
    <>
      ...
      <LookbookImageModal
        ...
        action={action}
        lookbook={lookbook}
        createLookbookImage={createLookbookImage}
        updateLookbookImage={updateLookbookImage}
        deleteLookbookImage={deleteLookbookImage}
        isLoading={isLoading}
        // isDeleted={isDeleted}
        setIsLoading={(state) => setIsLoading(state)}
      />
      ...
      <div className="grid grid-cols-12 gap-6 mt-5">
        <div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
          ...
          <div className="hidden md:block mx-auto text-slate-500">
            {/* Hiển thị {lookbooks.length} / {count} danh mục cha */}
            {/* Hiển thị {PAGE_SIZE} / {lookbooks.length}{" "} */}
