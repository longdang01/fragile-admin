...
import { lookbookImageModalValidator } from "../../common/Validation";
import {
  configSlugify,
...
const LookbookImageModal = (props) => {
  const initData = {
    _id: "",
    lookbook: props.lookbook._id,
    picture: "",
  };
  ...
  const [lookbookImage, setLookbookImage] = useState(initData);
  ...
  const handleImageRemove = (isReset) => {
    ...
    if (!isReset && ref.current) {
      ...
      if (confirm) {
        ref.current.value = "";
        setImage(initImage);
        setLookbookImage({ ...lookbookImage, ["picture"]: "" });
      }
    }
    ...
    if (isReset && ref.current) {
      ...
      setImage(initImage);
      setLookbookImage(initData);
    }
  };

  const getLookbookImage = async (id) => {
    const data = await LookbookImageService.getById(id);
    setLookbookImage(data.data);
  ...
  const onSave = async () => {
    const validate = lookbookImageModalValidator(lookbookImage);
    if (validate.error) {
    ...
    if (!validate.error) {
      ...
      lookbookImage.picture =
        uploaded && uploaded.data.data
          ? uploaded.data.data.url
          : lookbookImage.picture;
      if (!lookbookImage._id) createLookbookImage(lookbookImage);
      if (lookbookImage._id)
        updateLookbookImage(lookbookImage._id, lookbookImage);
  ...
  }, [lookbookImage, isChange]);

  useEffect(() => {
    handleImageRemove(true);
    setLabelInputs([]);
    setErrors([]);
    setTitle("Quản Lý Lookbook Ảnh");
      ...
      >
        <div className="height-[500px]">
          <div className="grid grid-cols-2 gap-5">
            <div className="g-col-12 form-group">
              ...
              <input
                ...
                required
                value={lookbookImage.picture || "Chưa có hình ảnh"}
                readOnly
              />
              <input
                type="file"
            ...
            <div className="g-col-12 form-group">
              ...
              <div>
                <button
                  ...
                  title={lookbookImage._id === "" ? "Thêm" : "Cập Nhật"}
                >
                  {/* {lookbookImage._id === "" ? "Thêm" : "Cập Nhật"} */}
                  {lookbookImage._id === "" ? (
                ...
                <button
                  ...
                  onClick={() => {
                    deleteLookbookImage(lookbookImage._id);
                    // handleImageRemove(true);
                    setIsChange(false);
                  }}
                  disabled={
                    lookbookImage._id === "" || props.isLoading ? true : false
                  }
                  title="Xóa"
                >
                  {/* Xóa */}
          ...
          <div>
            ...
            {lookbook.lookbookImages && lookbook.lookbookImages.length > 0 && (
              <Swiper
                ...
                {lookbook.lookbookImages.map((image, index) => (
                  <SwiperSlide key={index}>
                    <img
                      ...
                      onClick={(e) => getLookbookImage(image._id)}
                      alt=""
                    />
                  </SwiperSlide>
                ))}
...
export default LookbookImageModal;
